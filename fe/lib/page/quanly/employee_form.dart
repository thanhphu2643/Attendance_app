...
class _ManagerFormPageState extends State<ManagerFormPage> {
  ...
  void initState() {
    ...
    if (_ngaySinh.isNotEmpty) {
      _ngaySinhController.text =
          DateFormat('yyyy-MM-dd').format(DateTime.parse(_ngaySinh));
    }
  }
  ...
  Widget _buildFormField({required String label, required FormFieldSetter<
      String> onSaved, FormFieldValidator<
      String>? validator, bool obscureText = false, String? initialValue}) {
    ...
    return TextFormField(
      ...
      onTap: () async {
        DateTime? pickedDate = await showDatePicker(
          context: context,
          initialDate: _ngaySinh.isNotEmpty
              ? DateTime.parse(_ngaySinh)
              : DateTime.now(),
        ...
        if (pickedDate != null) {
          ...
          setState(() {
            _ngaySinh = DateFormat('yyyy-MM-dd').format(
                pickedDate); // Lưu vào biến _ngaySinh
            _ngaySinhController.text =
                _ngaySinh; // Cập nhật giá trị cho controller
          });
  ...
  Future<void> _saveInformation() async {
    if (_formKeyUser.currentState!.validate()) {
      ...
      Map<String, String> requestBody = {
        ...
        'gioiTinh': _gender,
        'ngaySinh': _ngaySinh,
        'maCongTy': widget.companyId.toString(),
      };
